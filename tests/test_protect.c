...
static int g_choice_answers[8], g_choice_count = 0, g_choice_next = 0;
...
static int btn_show_choice_alert(const char *title, const char *info, const char *a, const char *b) {
...
static void test_autosave(void) {
    ...
    CHECK(d->recovery_file == NULL && dir_entries(g_recovery_dir) <= 0, "clean doc: no recovery file");
    ...
    if (d->recovery_file) {
        char want[64];
        snprintf(want, sizeof want, "/%ld-%u.btnrecovery", (long)getpid(), d->recovery_id);
    ...
    CHECK(r.len == 11 && memcmp(r.text, "saved\ndraft", 11) == 0 && r.path && strcmp(r.path, a) == 0 &&
              r.eol == BTN_EOL_CRLF && !r.raw,
          "recovery holds text, path and line endings");
    btn_recovery_free(&r);
...
static void simulate_crash(void) {
    ...
    for (int i = 0; i < g_doc_count; i++) {
        ...
        char to[600];
        snprintf(to, sizeof to, "%s/%ld-%u.btnrecovery", g_recovery_dir, dead, d->recovery_id);
...
static void test_restore(void) {
    ...
    simulate_crash();
    snprintf(dmg, sizeof dmg, "%s/%ld-99.btnrecovery", g_recovery_dir, dead_pid());
    write_raw(dmg, "garbage");

    /* Lauf 2: Wiederherstellen */
    answers(1, 1);
    restore_recovered_documents();
    CHECK(g_choice_count == 1, "asked once");
    ...
    char **files;
    size_t n = btn_recovery_find_orphans(g_recovery_dir, (long)getpid(), &files);
    ...
    autosave_recovery((long)time(NULL) + 100); /* die Wiederherstellung oben schrieb mit echter Zeit */
    simulate_crash();
    ...
    answers(1, 1);
    restore_recovered_documents();
    n = btn_recovery_find_orphans(g_recovery_dir, (long)getpid(), &files);
    btn_recovery_free_list(files, n);
